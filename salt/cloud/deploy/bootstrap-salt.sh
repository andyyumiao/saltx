...
if [ "$_INSTALL_MINION" -eq $BS_FALSE ] && [ "$_INSTALL_MASTER" -eq $BS_FALSE ] && [ "$_INSTALL_SYNDIC" -eq $BS_FALSE ] && [ "$_INSTALL_MAID" -eq $BS_FALSE ] && [ "$_CONFIG_ONLY" -eq $BS_FALSE ]; then
...
if [ "$_INSTALL_MAID" -eq $BS_TRUE ]; then
    if [ "$_CONFIG_ONLY" -eq $BS_FALSE ]; then
        echoinfo "Installing maid"
    else
        echoinfo "Configuring maid"
    fi
fi
...
install_ubuntu_stable() {
    ...
    if [ "$_INSTALL_MAID" -eq $BS_TRUE ]; then
        __PACKAGES="${__PACKAGES} salt-maid"
    fi
...
install_ubuntu_stable_post() {
    for fname in api master minion syndic; do
        ...
        [ $fname = "maid" ] && [ "$_INSTALL_MAID" -eq $BS_FALSE ] && continue
...
install_debian_git_post() {
    for fname in api master minion syndic maid; do
        # Skip if not meant to be installed
        [ "$fname" = "api" ] && \
            ([ "$_INSTALL_MASTER" -eq $BS_FALSE ] || ! __check_command_exists "salt-${fname}") && continue
        [ "$fname" = "master" ] && [ "$_INSTALL_MASTER" -eq $BS_FALSE ] && continue
...
install_voidlinux_stable_post() {
    for fname in master minion syndic maid; do
        [ $fname = "master" ] && [ "$_INSTALL_MASTER" -eq $BS_FALSE ] && continue
        [ $fname = "minion" ] && [ "$_INSTALL_MINION" -eq $BS_FALSE ] && continue
        [ $fname = "syndic" ] && [ "$_INSTALL_SYNDIC" -eq $BS_FALSE ] && continue
        [ $fname = "maid" ] && [ "$_INSTALL_MAID" -eq $BS_FALSE ] && continue
...
config_salt() {
    ...
    if [ "$_INSTALL_MAID" -eq $BS_TRUE ] && [ "$_CONFIG_ONLY" -eq $BS_TRUE ]; then
        OVERWRITE_MASTER_CONFIGS=$BS_TRUE
    fi

    if [ "$_INSTALL_MASTER" -eq $BS_TRUE ] || [ "$_INSTALL_SYNDIC" -eq $BS_TRUE ] || [ "$_INSTALL_MAID" -eq $BS_TRUE ] || [ "$OVERWRITE_MASTER_CONFIGS" -eq $BS_TRUE ] || [ "$_CUSTOM_MASTER_CONFIG" != "null" ]; then
        # Create the PKI directory
