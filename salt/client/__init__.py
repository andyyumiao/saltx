...
class LocalClient(object):
    ...
    def cmd(self,
        ...
        try:
            ...
            #print('thethe local ret is: %s' % ret)
            return ret
        finally:
    ...
    def get_iter_returns(
        ...
        totalCheckCount = 0
        timeout_loop = time.time() + timeout
        calCount = 0
        while True:
                # if we got None, then there were
            ...
            currentCalCount = len(minions) - len(found.intersection(minions))

            if  currentCalCount == calCount:
                totalCheckCount = totalCheckCount + 1
            else:
                # NOTE: refresh check interval
                calCount = currentCalCount
                timeout_loop = time.time() + timeout
                if totalCheckCount > 0:
                    totalCheckCount = totalCheckCount - 1

            if time.time() - timeout_loop >= 0:
                if totalCheckCount >= 5:
                    break
