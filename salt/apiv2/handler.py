...
class LowDataAdapter(object):
    ...
    def exec_lowstate(self, client=None, token=None):
        ...
        for chunk in lowstate:
            ...
            api_log.info('----LowDataAdapter-----apiv2_run------:%s' % chunk)
    ...
    def exec_lowstate_aysnc(self, client=None, token=None):
        ...
        # Make any requested additions or modifications to each lowstate, then
        # execute each one and yield the result.
        logger.info('exec_lowstate_aysnc')
        logger.info(lowstate)
        for chunk in lowstate:
            if token:
                chunk['token'] = token
                if cherrypy.session.get('user'):
...
class Login(LowDataAdapter):
    ...
    def POST(self, **kwargs):
        ...
        **Example response:**
        ...
        referer = cherrypy.request.headers.get('Referer', '/').lower()
        api_log.info('------get-------referer1------:%s' % referer)
        ...
        api_log.info('-------apiv2 handler creds1----------%s' % creds)
        # Mint token.
        token = self.auth.mk_token(creds)
        api_log.info('-------apiv2 handler token1----------%s' % token)
